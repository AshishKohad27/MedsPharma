...
import { CLEAR_MESSAGE, LOGOUT, SAVE_CREDENTIAL } from "../../redux/auth/user.type";
...
function Login() {
    ...
    const { loading, error, isAuth, message, tokenDetails, errorMessage } = useSelector((store) => store.user);
    ...
    return (
        <>
            <div>
                ...
                <Menu variant='ghost'>
                    {isAuthUser ? <LogoutFunction name={tokenDetails && tokenDetails.name} /> : <MenuButton _hover={{ color: 'red' }} w='28' fontSize={'18'} righticon={<ChevronDownIcon />} onClick={onOpen}>
                        Hi, SignIn
                    </MenuButton>}
                    <MenuList>
...
function LogoutFunction({ name }) {
    ...
    return (
        <Flex w="160px" justifyContent="center" alignItems="center" >
            ...
            <Box>
                <Text ml="10px" color={"red.400"} fontWeight={500} cursor="pointer" onClick={() => dispatch({ type: LOGOUT })}>Logout</Text>
            </Box>
        </Flex>
    )
}
